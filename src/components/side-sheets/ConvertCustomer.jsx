...
const INPUT = "wiz-input w-full";
const LABEL = "block text-xs font-medium text-muted uppercase tracking-wider mb-1";
      ...
      {step === 1 ? (
        <>
          ...
          <div className="flex items-start gap-3 mb-5">
            <div className="w-9 h-9 rounded-full bg-success-bg flex items-center justify-center flex-shrink-0">
              <Star size={16} className="text-success-dark" />
            </div>
            <div>
              <p className="text-sm text-muted">
                Convert <strong className="text-ink">{company?.name}</strong> from a{" "}
          ...
          <div className="flex-1 space-y-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer pt-1">
              ...
              />
              <span className="text-sm text-muted">Grant WizShop access to moved contacts</span>
            </label>

            {/* Contact-movement behavior hint (driven by org setting) */}
            <div className="flex items-start gap-2 text-xs text-muted bg-default border border-border rounded-lg px-3 py-2">
              <Info size={13} className="text-disabled flex-shrink-0 mt-0.5" />
              <span>{movementHint}</span>
            </div>
          </div>

          {/* Footer */}
          <div className="pt-4 mt-4 border-t border-divider flex items-center gap-2">
            <button
              onClick={handleStep1Submit}
              className="wiz-btn flex-1 bg-success text-white hover:bg-success-dark border-success"
            >
        ...
        <>
          <div className="flex items-start gap-3 mb-4">
            <div className="w-9 h-9 rounded-full bg-tonal flex items-center justify-center flex-shrink-0">
              <Users size={16} className="text-primary" />
            </div>
            <div>
              <p className="text-sm text-muted">
                Select which contacts to move to <strong className="text-ink">{company?.name}</strong> (Customer).
              </p>
              <p className="text-xs text-disabled mt-0.5">
          ...
          <div className="flex-1 overflow-y-auto space-y-2">
            {companyContacts.length === 0 && (
              <div className="text-sm text-disabled text-center py-6">This company has no contacts.</div>
            )}
              ...
              return (
                <label
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                    checked ? "border-primary bg-tonal" : "border-border hover:border-primary"
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-ink truncate">{contactName(c)}</div>
                    <div className="text-xs text-disabled truncate">{c.email}</div>
                  </div>
                  {c.isWizShopUser && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-tonal text-primary flex-shrink-0">WizShop</span>
                  )}
                </label>
              );
            })}
          ...
          <div className="pt-4 mt-4 border-t border-divider flex items-center gap-2">
            ...
            <button onClick={() => setStep(1)} className="wiz-btn wiz-btn--text">
              Back
            </button>
          </div>
        </>
