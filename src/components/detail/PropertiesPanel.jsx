...
export default function PropertiesPanel({ groups, values, onChange }) {
  return (
    <div className="w-72 border-r border-gray-100 overflow-y-auto bg-gray-50/50 p-4 flex-shrink-0">
      {groups.map((group) => (
  ...
  if (field.type === "address") {
    return (
      <div>
        ...
        {editing ? (
          <div className="space-y-1.5 bg-white border border-indigo-200 rounded p-2">
            {["street", "city", "state", "country", "zip"].map((part) => (
              <input
                ...
                className="w-full text-sm border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-300"
              />
            ))}
            <InlineActions onSave={save} onCancel={cancel} />
          </div>
        ) : (
          <div className="group relative text-sm text-gray-900 bg-white border border-gray-200 rounded px-2.5 py-1.5">
        ...
        <div className="flex items-center gap-1">
          {field.type === "select" ? (
            <select
              ...
              className="flex-1 text-sm bg-white border border-indigo-200 rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-300"
            >
