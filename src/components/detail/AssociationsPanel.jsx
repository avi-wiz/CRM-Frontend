...
const ROLE_COLOR = {
  "Decision Maker": "bg-action-selected text-primary-dark",
  Billing: "bg-warning-bg text-warning-dark",
  User: "bg-tonal text-muted",
};
...
export default function AssociationsPanel({ company, stages, stage, onStageChange, onContactClick, onDealClick, onAction }) {
  ...
  return (
    <div className="w-64 border-l border-border overflow-y-auto bg-surface p-5 flex-shrink-0">
      {/* Contacts */}
      <Block title="Contacts" icon={Users} action={{ label: "+ Add Contact", onClick: () => onAction?.("addContact") }}>
        {(company.contacts || []).map((c) => (
          <div key={c.id} onClick={() => onContactClick?.(c)} className="flex items-center gap-2.5 py-2 px-2 rounded-xl hover:bg-action-hover cursor-pointer transition-colors duration-150">
            <div className="w-7 h-7 rounded-full bg-primary flex items-center justify-center text-[10px] font-extrabold text-white flex-shrink-0 shadow-1">
              {initials(c.name)}
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-medium text-ink leading-tight truncate">{c.name}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0 ${ROLE_COLOR[c.role] || "bg-tonal text-muted"}`}>{c.role}</span>
              </div>
      ...
      <Block title="Deals" icon={DollarSign} action={{ label: "+ Create Deal", onClick: () => onAction?.("addDeal") }}>
        {(company.deals || []).map((d) => (
          <div key={d.id} onClick={() => onDealClick?.(d)} className="py-2 px-2.5 rounded-xl border border-transparent hover:border-tonal hover:bg-action-hover cursor-pointer transition-all duration-150">
            <div className="text-sm font-medium text-ink">{d.name}</div>
            <div className="flex items-center gap-2 mt-1">
              <StageBadge stage={d.stage} small />
              <span className="text-xs font-semibold text-muted">{d.amount}</span>
            </div>
          </div>
        ))}
      </Block>
      ...
      <Block title="Pipeline" icon={GitBranch}>
        ...
        <label className="text-xs text-muted block mt-2 mb-1">Move to Stage</label>
        <select
          value={stage}
          onChange={(e) => onStageChange?.(e.target.value)}
          className="wiz-input w-full text-sm px-2 py-1.5"
        >
      ...
      <Block title="WizShop Users" icon={Globe} action={{ label: "+ Grant Access", onClick: () => onAction?.("grantAccess") }}>
        {wizshopUsers.length === 0 && <div className="text-xs text-disabled py-1">No WizShop users</div>}
        {wizshopUsers.map((c) => (
          <div key={c.id} className="flex items-center justify-between py-1 px-2 rounded hover:bg-action-hover">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0 ${c.wizshopStatus === "Active" ? "bg-success-bg text-success-dark" : "bg-tonal text-muted"}`}>
              {c.wizshopStatus}
            </span>
          </div>
        ))}
      ...
      <Block title="Parent / Child" icon={Network} action={{ label: "+ Link Company", onClick: () => onAction?.("linkCompany") }}>
        <div className="text-xs text-muted mb-1">
          Parent: <span className="text-ink">{company.parent?.name || "No parent company"}</span>
        </div>
        {(company.children || []).length > 0 ? (
          <div className="space-y-0.5">
            {company.children.map((c) => (
              <div key={c.id} className="text-xs text-muted px-2 py-1 rounded hover:bg-action-hover cursor-pointer">↳ {c.name}</div>
            ))}
          </div>
        ) : (
          <div className="text-xs text-disabled">No child companies</div>
        )}
      </Block>
    </div>
  );
  ...
  return (
    <div className="flex items-start justify-between gap-2 py-1">
      <div className="min-w-0">
        <div className="text-[10px] uppercase tracking-wide text-disabled">{label}</div>
        <div className="text-xs text-muted truncate">{text}</div>
      </div>
      <button onClick={onEdit} className="text-xs text-primary hover:text-primary-dark flex-shrink-0">Edit</button>
    </div>
  );
}
...
function Block({ title, icon: Icon, action, children }) {
  return (
    <div className="mb-6 pb-5 border-b border-divider last:border-0 last:pb-0">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          {Icon && <Icon size={13} className="text-disabled" />}
          <h4 className="text-[10px] font-bold text-disabled uppercase tracking-widest">{title}</h4>
        </div>
        {action && (
...
function CardRow({ card }) {
  return (
    <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-border bg-default">
      <CreditCard size={13} className="text-disabled flex-shrink-0" />
      <span className="text-xs text-muted font-mono tracking-tight">
        XXXX XXXX XXXX {card.last4}
      </span>
      {card.brand && <span className="text-[10px] text-disabled ml-auto">{card.brand}</span>}
    </div>
  );
}
